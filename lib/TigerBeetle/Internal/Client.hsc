...
tigerbeetleTypesTable =
  Map.fromList
    [ (C.TypeName "TB_ACCOUNT_FLAGS", [t| AccountFlags |])
    , (C.TypeName "tb_account_t", [t| Account |])
    , (C.TypeName "TB_TRANSFER_FLAGS", [t| TransferFlags |])
    , (C.TypeName "tb_transfer_t", [t| Transfer |])
    , (C.TypeName "TB_CREATE_ACCOUNT_RESULT", [t| CreateAccountResult |])
    , (C.TypeName "TB_CREATE_TRANSFER_RESULT", [t| CreateTransferResult |])
    , (C.TypeName "TB_CREATE_ACCOUNTS_RESULT", [t| CreateAccountsResult |])
    , (C.TypeName "TB_CREATE_TRANSFERS_RESULT", [t| CreateTransfersResult |])
    , (C.TypeName "tb_account_filter_t", [t| AccountFilter |])
    , (C.TypeName "TB_ACCOUNT_FILTER_FLAGS", [t| AccountFilterFlags |])
    , (C.TypeName "tb_account_balance_t", [t| AccountBalance |])
    , (C.TypeName "tb_query_filter_t", [t| QueryFilter |])
    , (C.TypeName "TB_QUERY_FILTER_FLAGS", [t| QueryFilterFlags |])
    , (C.TypeName "tb_client_t", [t| TBClient |])
    , (C.TypeName "tb_packet_t", [t| Packet |])
    , (C.TypeName "TB_OPERATION", [t| Operation |])
    , (C.TypeName "TB_PACKET_STATUS", [t| PacketStatus |])
    , (C.TypeName "TB_INIT_STATUS", [t| InitStatus |])
    , (C.TypeName "TB_CLIENT_STATUS", [t| ClientStatus |])
    , (C.TypeName "TB_REGISTER_LOG_CALLBACK_STATUS", [t| RegisterLogCallbackStatus |])
    , (C.TypeName "TB_LOG_LEVEL", [t| LogLevel |])
    ]